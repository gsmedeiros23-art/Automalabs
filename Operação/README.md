...
Você acaba de instalar o Automalabs. Em alguns minutos, sua empresa vai
...
```
Clona o https://github.com/mazzeoia/Automalabs.git na pasta atual,
...
```
git clone https://github.com/mazzeoia/Automalabs.git
cd Automalabs
code .
```
...
Quando o `/instalar` terminar, renomeia a pasta `Automalabs/` pro nome do teu
negócio (fecha o VS Code, renomeia no Explorer/Finder, abre de novo). A
pasta não fica como "Automalabs" — ela é o teu negócio agora.
...
closed loop dentro do Automalabs (decide → executa → captura → realimenta →
ajusta sozinho).
